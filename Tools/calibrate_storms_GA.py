...
def storm_fitness_overwash(solution, solution_idx):
...
def storm_fitness_beach(solution, solution_idx):
    """Run a storm with this particular combintion of parameter values, and return fitness value of simulated to observed."""
...
overwash = True  # [bool] Determines whether to calibrate either overwash [True] or beach/dune [False] parameters. Use calibrate_storms_GA_all.py to calibrate both together.
...
if overwash:
    fitness_function = storm_fitness_overwash
    num_genes = 7
    gene_type = [int,
                 int,
                 [float, 2],
    ...
    fitness_function = storm_fitness_beach
    num_genes = 3
    gene_type = [[float, 3],
                 [float, 2],
                 int]
    gene_space = [{'low': 0.002, 'high': 0.2},  # Beq
                  {'low': 0.25, 'high': 4},  # Et
...
if overwash:
    ...
    print(tabulate({
        ...
        }, headers="keys", floatfmt=(None, ".0f", ".0f", ".2f", ".2f", ".7f", ".2f", ".0f", ".4f"))
    )
else:  # Beach/Dune
    print()
    print(tabulate({
        ...
        "SSb":  [best_solution[2]],
        "Score": [solution_fitness]
        }, headers="keys", floatfmt=(None, ".3f", ".2f", ".0f", ".4f"))
    )
